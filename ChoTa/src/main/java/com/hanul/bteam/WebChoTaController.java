...
@Controller
public class WebChoTaController {
...
//	}
	...
	public boolean id_check(String userid) {
		...
		return dao.member_id_check(userid) == 0? false : true;
	}
...
//		Authorization: Bearer ${ACCESS_TOKEN}
		...
		if( ! json.isEmpty() ) {
			...
			//카카오 아이디가 있는지 확인하여 
			//카카오로그인이 처음이면  insert(0), 아니면 update(1)
			if( dao.member_id_check(vo.getUserid()) == 1 ) {
...
//		API 요청 실패시 : http://콜백URL/redirect?state={state값}&error={에러코드값}&error_description={에러메시지}
		...
		if( json.getString("resultcode").equals("00") ) {
			...
			//vo.setEmail( json.getString("email") ); 우린 없음
	...
	//지능형iot회원 로그인처리 요청
	@ResponseBody @RequestMapping("/chotalogin")
	public boolean login(String id, String pw, HttpSession session) {
		...
		//String salt = dao.member_salt(userid);
		//userpw = common.getEncrypt(salt, userpw); //암호화된 비번
		...
		map.put("userpw", pw);
		map.put("userid", id);		
		MemberVO vo = dao.login(map, social);
